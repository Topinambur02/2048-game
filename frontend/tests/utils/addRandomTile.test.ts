...
describe('addRandomTile', () => {
    ...
    test('should add a tile (2) to a random empty cell (90% chance)', () => {
        const board = [[0, 2],[2, 0]];
        (getEmptyCells as jest.Mock).mockReturnValue([[0, 0], [1, 1]]);
    ...
    test('should add a tile (4) to a random empty cell (10% chance)', () => {
        const board = [[2, 0], [0, 2]];
        (getEmptyCells as jest.Mock).mockReturnValue([[0, 1], [1, 0]]);
