...
describe('useAudio', () => {
    ...
    test('should initialize with default volume and create sound instance', async () => {
        const { result } = renderHook(() => useAudio(mockSoundSource, baseOptions))
    ...
    test('should recreate sound when shouldPlay changes', async () => {
        const { rerender } = renderHook(({ options }: { options: OptionsType }) => useAudio(mockSoundSource, options), {
    ...
    test('should unload sound on unmount', async () => {
        const { unmount } = renderHook(() => useAudio(mockSoundSource, baseOptions))
    ...
    test('should handle creation errors', async () => {
        ...
        ;(Audio.Sound.createAsync as jest.Mock).mockRejectedValue(new Error('Creation error'))

        await act(async () => {
            renderHook(() => useAudio(mockSoundSource, baseOptions))
        })

        expect(consoleSpy).toHaveBeenCalledWith('Ошибка воспроизведения:', expect.any(Error))
    })

    test('should handle volume update errors', async () => {
        const consoleSpy = jest.spyOn(console, 'log')
        mockSoundObject.setVolumeAsync.mockRejectedValueOnce(new Error('Volume error'))
        ...
        expect(consoleSpy).toHaveBeenCalledWith('Ошибка изменения громкости:', expect.any(Error))
    })
